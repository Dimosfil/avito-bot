...
| Backend/API | FastAPI | `app/main.py`, `app/runtime_state.py`, `app/autoreply_logic.py`, `app/manager_notifications.py`, `app/admin_logging.py`, `pyproject.toml` | Serves API and static UI. Routes stay in `app/main.py`; runtime-state, auto-reply helper logic, admin/runtime logging, and Telegram notification helpers are separate modules. |
